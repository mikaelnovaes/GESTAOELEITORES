 ...
 * Multi-tenant + correção do bug "template excluído bloqueia novo nome"
 */
...
/* ── DELETE /api/whatsapp/templates/:id ──────────────────────
   CORREÇÃO DO BUG #6 (template excluído bloqueando nome):
   Agora faz HARD delete. Templates da Meta são externos —
   não precisamos manter histórico local soft-deleted.
   Logs de envio continuam preservados (referenciam pelo nome no JSON do payload).
─────────────────────────────────────────────────────────── */
...
function buildMessageBody(phone, mode, payload, eleitor) {
  ...
  if (mode === 'text') {
    // Aceita tanto 'message' (vindo dos robôs) quanto 'text' (vindo do envio manual)
