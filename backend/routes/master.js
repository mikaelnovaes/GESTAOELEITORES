  ...
  async (req, res) => {
    ...
    try {
      const created = await db.transaction(async (client) => {
        ...
        const t = r.rows[0];
        // Configs dos robôs criadas atomicamente junto com o tenant
        await client.query(
      ...
      await logMasterAction(req, 'TENANT_CREATE', { tenant_id: Number(created.id), nome, slug });
      res.status(201).json({ id: Number(created.id), slug: created.slug, criado_em: created.criado_em });
...
router.delete('/tenants/:id',
  ...
  async (req, res) => {
    ...
    try {
      // Reautenticação
      const u = await db.query('SELECT senha_hash FROM usuarios WHERE id = $1 AND ativo = TRUE', [req.user.id]);
      if (!u.rowCount) return res.status(401).json({ error: 'Master não encontrado ou inativo.' });
      ...
      // DELETE em transação (CASCADE removerá eleitores, configs, logs etc.)
      ...
      if (!deleted.rowCount) return res.status(404).json({ error: 'Ambiente não encontrado.' });

      await logMasterAction(req, 'TENANT_DELETE', {
        tenant_id: req.params.id,
        nome: deleted.rows[0].nome,
...
router.put('/me/senha',
  ...
  async (req, res) => {
    ...
    try {
      const u = await db.query('SELECT senha_hash FROM usuarios WHERE id = $1 AND ativo = TRUE', [req.user.id]);
      if (!u.rowCount) return res.status(401).json({ error: 'Master não encontrado.' });
