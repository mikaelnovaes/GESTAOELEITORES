 ...
 * CRUD completo de eleitores — PostgreSQL
 */
...
router.get('/',
  ...
  async (req, res) => {
    ...
    try {
      // PostgreSQL: ILIKE para busca case-insensitive sem collation especial
...
router.post('/',
  [
    ...
    body('email').optional({ nullable: true }).isEmail().isLength({ max: 200 }),
    body('telefone').optional({ nullable: true }).isLength({ max: 20 }),
    body('data_nascimento').optional({ nullable: true }).isDate(),
    ...
    try {
      const r = await db.query(
        `INSERT INTO eleitores
           (nome, data_nascimento, telefone, email, endereco, numero,
    ...
    try {
      const r = await db.query(
        `UPDATE eleitores SET ativo = FALSE, atualizado_em = NOW()
         WHERE id = $1 AND ativo = TRUE RETURNING id`,
        [req.params.id]
      );
      if (!r.rowCount) return res.status(404).json({ error: 'Eleitor não encontrado.' });
      res.json({ success: true });
      ...
      await db.transaction(async (client) => {
        for (const raw of records) {
          ...
          try {
            await client.query(
              `INSERT INTO eleitores
                 (nome, telefone, email, endereco, numero, bairro, cidade, criado_por)
  ...
  try {
    const r = await db.query(`
      SELECT
        ...
        COUNT(DISTINCT bairro) FILTER (WHERE bairro IS NOT NULL) AS bairros,
        COUNT(DISTINCT cidade) FILTER (WHERE cidade IS NOT NULL) AS cidades
