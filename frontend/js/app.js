/**
 * frontend/js/app.js v3.0
 * - API é fonte da verdade
 * - syncFromAPI sempre força reload (resolve "exclui via SQL não some")
 * - IDs normalizados para Number (resolve botões)
 * - Botão "Excluir TODOS" com 3 confirmações + senha
 * - Robôs com formulário completo via API
 */
...
function saveSession(token, user) {
  ...
  sessionStorage.setItem('ge_user', JSON.stringify({
    id: Number(user.id), tipo: user.tipo, nome: user.nome, tenant_id: user.tenant_id
  }));
}
...
function clearSession() {
  ...
  currentUser = null;
  // limpa cache de eleitores ao deslogar
  if (window.Eleitores && window.Eleitores.clear) window.Eleitores.clear();
}
...
/* ============================================================
   SINCRONIZAÇÃO COM API — sempre substitui o cache
   ============================================================ */
async function syncFromAPI() {
  try {
    ...
    // SEMPRE substitui o cache local — mesmo que vazio
          ...
          return `
            <tr>
              <td>${e.foto_url
                ...
                : `<div class="row-photo-placeholder">${escapeHtml((e.nome[0] || '?').toUpperCase())}</div>`}</td>
...
/* ============================================================
   ...
   ============================================================ */
document.getElementById('btn-new-eleitor')?.addEventListener('click', () => {
  openEleitorForm(null);
});
...
function openDetailModal(e) {
  ...
  body.innerHTML = `
    <div style="display:flex;gap:1.4rem;align-items:flex-start;flex-wrap:wrap;">
      ${e.foto_url
        ? `<img src="${escapeHtml(e.foto_url)}" style="width:120px;height:120px;border-radius:8px;object-fit:cover">`
        : `<div style="width:120px;height:120px;border-radius:8px;background:var(--cream);display:flex;align-items:center;justify-content:center;font-family:'Fraunces',serif;font-size:3rem;color:var(--navy);">${escapeHtml((e.nome[0] || '?').toUpperCase())}</div>`}
      <div style="flex:1;">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.8rem;margin-top:1.4rem;">
      ${[
        ...
        ['Título', e.titulo_eleitor], ['Seção', e.secao], ['Local', e.escola_votacao],
...
document.getElementById('btn-purge-all')?.addEventListener('click', async () => {
  // 1ª confirmação
  if (!confirm('⚠️ ATENÇÃO: Isso vai EXCLUIR TODOS OS ELEITORES da sua base. Continuar?')) return;
  // 2ª confirmação: digitar palavra-chave
  ...
  try {
    ...
    showToast(`✓ ${r.excluidos} eleitor(es) excluído(s) permanentemente.`, 'success');
    await syncFromAPI();
    renderList();
  } catch (err) {
    showToast(err.message || 'Erro na exclusão em massa.', 'error');
  }
});
  ...
  try {
    ...
    container.querySelectorAll('[data-uact]').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        if (act === 'delete') {
          if (uid === Number(currentUser?.id)) {
            showToast('Não pode excluir seu próprio usuário.', 'error'); return;
          }
...
document.getElementById('user-form')?.addEventListener('submit', async (e) => {
  ...
  // Só inclui senha se foi preenchida (corrige bug edição admin)
...
window.addEventListener('load', async () => {
  try {
    if (window.GESecurity?.RateLimiter) window.GESecurity.RateLimiter.reset('login');
  } catch(e) {}
...
/* ============================================================
   VARREDURA DE DUPLICATAS (sincroniza antes)
   ============================================================ */
...
document.getElementById('btn-check-duplicates')?.addEventListener('click', async () => {
  ...
  if (!modal) return;
  document.getElementById('dup-modal-body').innerHTML = '<div class="empty" style="padding:3rem;"><p style="color:var(--muted);">Sincronizando com servidor...</p></div>';
  modal.classList.add('show');

  // Sincroniza antes (ponto 16)
  await syncFromAPI();

  const eleitores = Eleitores.all();
  const groups = detectDuplicates(eleitores);
...
function renderDuplicatesModal(groups) {
  ...
  if (!body) return;
  if (!groups.length) {
    body.innerHTML = `<div class="empty" style="padding:3rem;"><h3 style="color:var(--success)">Nenhuma duplicata encontrada!</h3></div>`;
  ...
  footer.innerHTML = `
    ...
    <button class="btn btn-danger" id="btn-confirm-remove-dup">Remover ${totalToRemove} Duplicata(s)</button>`;

  document.getElementById('btn-confirm-remove-dup').addEventListener('click', async function() {
    if (!confirm(`Remover ${totalToRemove} duplicata(s)? Esta ação não pode ser desfeita.`)) return;
    this.disabled = true;
    ...
    for (const group of groups) {
      ...
      for (const e of group) {
        ...
        try {
          await API.delete(`/eleitores/${e.id}`);
          removed++;
        } catch (err) { console.warn('Falha ao excluir', e.id, err.message); }
      }
    }
...
// rebind dos data-close (caso o footer foi reescrito)
