...
document.getElementById('login-form')?.addEventListener('submit', async (e) => {
  ...
  // Rate limiting no frontend (segurança secundária — a primária é no backend)
  const rl = window.GESecurity?.RateLimiter;
  if (rl?.isBlocked('login')) {
    errEl.textContent = 'Muitas tentativas. Aguarde 15 minutos.';
    errEl.classList.add('show');
    document.getElementById('login-rate-warning').style.display = 'block';
    return;
  }
...
window.addEventListener('load', async () => {
  ...
  // Inicializar módulos
  window.GEImport?.initImport();
  window.GEWhatsApp?.initWhatsApp();

  // Iniciar robôs
  window.GERobots?.startBirthdayWatcher();
  window.GERobots?.startReactivationWatcher();
});
